...
func TestDialerRejectsItsOwnMirroredRequest(t *testing.T) {
	...
	sent := append([]byte{tagToListener}, frame...)
	...
	if _, ok := stripTag(data, tagToDialer); ok {
	...
	good := append([]byte{tagToDialer}, frame...)
	if got, ok := stripTag(good, tagToDialer); !ok || !bytes.Equal(got, frame) {
...
func TestListenerIgnoresOrdinaryPing(t *testing.T) {
	if _, ok := stripTag([]byte("abcdefgh"), tagToListener); ok {
		t.Fatal("a plain ping payload was accepted as a tunnel frame")
	}
	if _, ok := stripTag(nil, tagToListener); ok {
		t.Fatal("an empty payload was accepted as a tunnel frame")
	}
	if _, ok := stripTag([]byte{tagToDialer}, tagToListener); ok {
